...
void ModbusAdapter::modbusWriteData(int slave, int functionCode, int startAddress, int noOfItems)
{
    ...
    {
            case MODBUS_FC_WRITE_SINGLE_COIL:
                    QLOG_ERROR() <<regModel->value(0)<<  "MODBUS_FC_WRITE_SINGLE_COIL "<<startAddress << "srinivas";
                    ret = modbus_write_bit(m_modbus, startAddress,regModel->value(0));
                    noOfItems = 1;
                    break;

            case MODBUS_FC_WRITE_SINGLE_REGISTER:
                    QLOG_ERROR() <<regModel->value(0)<<  "MODBUS_FC_WRITE_SINGLE_REGISTER "<<startAddress << "srinivas";
