    ...
    ui(new Ui::MainWindow)
{
    ...
    qDebug() << "Srinivas ---- " << m_modbusCommSettings->functionCode();
...
void MainWindow::changedConnect(bool value, QString portName)
{
    QString string = getTextEdit()->toPlainText();
    getTextEdit()->setPlainText(QString("%1 changedConnect %2 PortName %3 ").arg(string).arg(portName).arg(value));
    if (value) { //Connected
        modbusConnect(true, portName);
        QLOG_INFO()<<  "Connected ";
    }
    else { //Disconnected
        modbusConnect(false);
...
void MainWindow::changedStartAddress(int value)
{
    ...
    QLOG_ERROR()<<  "Start Address changed. Value = " << value;
...
void MainWindow::setSBStartAddValue(int value, int type)
{
    ui->cmbStartAddrBase->setCurrentIndex(type);
    ui->sbStartAddress->setValue(value);
}
...
void MainWindow::modbusConnect(bool connect, QString portName)
 {

    //Modbus connect - RTU/TCP
    QLOG_TRACE()<<  "Modbus Connect. Value = " << connect;
    if (portName.isEmpty())
        portName = m_modbusCommSettings->serialPortName();

    QString string = getTextEdit()->toPlainText();
    getTextEdit()->setPlainText(QString("%1 PortName %2").arg(string).arg(portName));
...
bool MainWindow::isModBusConnected()
{
